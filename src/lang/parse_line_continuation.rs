use crate::{error::parse_error::ParseError, NEW_LINE_CODE};
...
pub(crate) fn set_results(content: &mut String, params: &mut Vec<String>, r_content: &str, r_params : &mut Vec<String>) {
...
pub(crate) fn parse_line_continuation(pair: Pair) -> Result<LineResult, ParseError> {
    match pair.as_rule() {
        Rule::normal_end =>{
			Ok(LineResult{ 
				content : String::new(),
...
fn parse_normal_end(mut pairs: Pairs) -> Result<Params, ParseError> {
...
fn parse_continuation(mut pairs: Pairs) -> Result<Params, ParseError> {
    let _tabs = pairs.next();
    parse_continued_line_without_content(pairs)
}

fn parse_backslash_comment_end(mut pairs: Pairs) -> Result<LcnResult, ParseError> {
    let _comment = pairs.next();
	parse_backslash_end(pairs)
}

fn parse_backslash_end(mut pairs: Pairs) -> Result<LcnResult, ParseError> {
...
fn parse_single_bar(mut pairs : Pairs) -> Result<LineResult, ParseError>{
	let _new_line = pairs.next();
	parse_continued_line(pairs)
}

fn parse_double_bars(mut pairs : Pairs) -> Result<Params, ParseError>{
	let _comment = pairs.next();
	parse_normal_end(pairs)
}

fn parse_triple_bars(mut pairs : Pairs) -> Result<LineResult, ParseError>{
	let _comment = pairs.next();
	parse_single_bar(pairs)
}

fn parse_continued_line(mut pairs: Pairs) -> Result<LineResult, ParseError> {
    let _tabs = pairs.next();
...
fn parse_continued_line_with_content(
    mut pairs: Pairs,
) -> Result<LineResult, ParseError> {
...
fn parse_continued_line_without_content(mut pairs: Pairs) -> Result<Params, ParseError> {
    let mut vec = vec![];
